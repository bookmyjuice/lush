...
void main() {
  ...
  setUp(() { TestWidgetsFlutterBinding.ensureInitialized(); mockRepo = MockUserRepository(); });
  ...
  group('AutoLogIn', () {
    ...
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      act: (bloc) => bloc.add(AutoLogIn()),
      expect: () => [isA<AuthenticationInProgress>(), isA<InternetIssue>()],
    );
  });
  ...
  group('LogOut', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () { when(() => mockRepo.logout()).thenAnswer((_) async => {}); return AuthenticationBloc(repo: mockRepo); },
      act: (bloc) => bloc.add(LogOut()),
      expect: () => [isA<AuthenticationInProgress>(), isA<LoggedOut>()],
    );
  });
  ...
  group('ChooseSignupMethod', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      verify: (bloc) { expect((bloc.state as SignupMethodSelected).method, 'phone'); },
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      expect: () => [isA<SignupMethodSelected>()],
      verify: (bloc) { expect((bloc.state as SignupMethodSelected).method, 'email'); },
    );
  });
  ...
  group('EnterEmail', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () {
        when(() => mockRepo.sendEmailVerification(any<String>())).thenAnswer((_) async => 'Success: Code sent');
        return AuthenticationBloc(repo: mockRepo);
      },
      act: (bloc) => bloc.add(const EnterEmail(email: 'test@example.com')),
      expect: () => [isA<EmailEntered>(), isA<EmailVerificationCodeSent>()],
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
      'EnterEmail failure emits EmailEntered then EmailVerificationFailed',
      build: () {
        when(() => mockRepo.sendEmailVerification(any<String>())).thenAnswer((_) async => 'Error: Already registered');
        return AuthenticationBloc(repo: mockRepo);
      },
      act: (bloc) => bloc.add(const EnterEmail(email: 'test@example.com')),
      expect: () => [isA<EmailEntered>(), isA<EmailVerificationFailed>()],
    );
  });
  ...
  group('VerifyEmail', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () {
        when(() => mockRepo.sendEmailVerification(any<String>())).thenAnswer((_) async => 'Success: Code sent');
        when(() => mockRepo.verifyEmailCode(any<String>(), any<String>())).thenAnswer((_) async => 'Success: Email verified');
        return AuthenticationBloc(repo: mockRepo);
      },
      act: (bloc) async {
        bloc.add(const EnterEmail(email: 'test@example.com'));
        await Future<void>.delayed(const Duration(milliseconds: 10));
        bloc.add(const VerifyEmail(email: 'test@example.com', verificationCode: '123456'));
      },
      expect: () => [isA<EmailEntered>(), isA<EmailVerificationCodeSent>(), isA<EmailVerified>()],
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
      'VerifyEmail wrong code fails',
      build: () {
        when(() => mockRepo.sendEmailVerification(any<String>())).thenAnswer((_) async => 'Success: Code sent');
        when(() => mockRepo.verifyEmailCode(any<String>(), any<String>())).thenAnswer((_) async => 'Error: Invalid code');
        return AuthenticationBloc(repo: mockRepo);
      },
      act: (bloc) async {
        bloc.add(const EnterEmail(email: 'test@example.com'));
        await Future<void>.delayed(const Duration(milliseconds: 10));
        bloc.add(const VerifyEmail(email: 'test@example.com', verificationCode: 'wrongcode'));
      },
      expect: () => [isA<EmailEntered>(), isA<EmailVerificationCodeSent>(), isA<EmailVerificationFailed>()],
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
      'VerifyEmail with mismatched email fails immediately',
      build: () {
        when(() => mockRepo.sendEmailVerification(any<String>())).thenAnswer((_) async => 'Error: Invalid email');
        return AuthenticationBloc(repo: mockRepo);
      },
      act: (bloc) async {
        bloc.add(const EnterEmail(email: 'different@example.com'));
        await Future<void>.delayed(const Duration(milliseconds: 10));
        bloc.add(const VerifyEmail(email: 'other@example.com', verificationCode: '123456'));
      },
      expect: () => [isA<EmailEntered>(), isA<EmailVerificationFailed>(), isA<EmailVerificationFailed>()],
    );
  });
  ...
  group('SendOTP', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () { when(() => mockRepo.sendOTP(any<String>())).thenAnswer((_) async => 'OTP_SENT'); return AuthenticationBloc(repo: mockRepo); },
    ...
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () { when(() => mockRepo.sendOTP(any<String>())).thenAnswer((_) async => 'Error: Invalid phone number'); return AuthenticationBloc(repo: mockRepo); },
  ...
  group('VerifyOTP', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () { when(() => mockRepo.verifyOTP(any<String>())).thenAnswer((_) async => 'OTP_VERIFIED'); return AuthenticationBloc(repo: mockRepo); },
    ...
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () { when(() => mockRepo.verifyOTP(any<String>())).thenAnswer((_) async => 'Error: Invalid OTP'); return AuthenticationBloc(repo: mockRepo); },
      act: (bloc) => bloc.add(const VerifyOTP(otp: 'wrong', phone: '')),
      expect: () => [isA<OTPVerificationFailed>()],
    );
  });

  // ─── EnterAddress — BLoC emits ReadyForFinalSignup directly (no AddressEntered interim) ─────
  group('EnterAddress', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      'EnterAddress emits ReadyForFinalSignup',
      build: () => AuthenticationBloc(repo: mockRepo),
      act: (bloc) => bloc.add(const EnterAddress(firstName: 'Test', lastName: 'User', address: '123 Test St', extendedAddr: '', extendedAddr2: '', city: 'Mumbai', state: 'Maharashtra', zip: '400001', country: 'IN')),
      expect: () => [isA<ReadyForFinalSignup>()],
      verify: (bloc) {
  ...
  group('CompleteSignup', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () { when(() => mockRepo.user).thenReturn(createTestUser()); return AuthenticationBloc(repo: mockRepo); },
    ...
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      expect: () => [isA<ReadyForFinalSignup>(), isA<AuthenticationInProgress>(), isA<AuthenticationSuccess>()],
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
  ...
  group('GoogleSignIn', () {
    blocTest<AuthenticationBloc, AuthenticationState>(
      ...
      build: () { when(() => mockRepo.googleSignIn()).thenAnswer((_) async => {'type':'login_success','token':'mock-token'}); when(() => mockRepo.user).thenReturn(createTestUser()); return AuthenticationBloc(repo: mockRepo); },
      act: (bloc) => bloc.add(const GoogleSignIn()),
      expect: () => [isA<AuthenticationSuccess>()],
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
      'link_required emits GoogleLinkRequired',
      build: () { when(() => mockRepo.googleSignIn()).thenAnswer((_) async => {'type':'link_required','googleEmail':'test@google.com','googleFirstName':'Test','googleLastName':'User','googleId':'google-123','photoUrl':null}); return AuthenticationBloc(repo: mockRepo); },
      act: (bloc) => bloc.add(const GoogleSignIn()),
      expect: () => [isA<GoogleLinkRequired>()],
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
      'signup_required emits SignUpStarted',
      build: () { when(() => mockRepo.googleSignIn()).thenAnswer((_) async => {'type':'signup_required','user':createTestUser()}); when(() => mockRepo.user).thenReturn(createTestUser()); return AuthenticationBloc(repo: mockRepo); },
      act: (bloc) => bloc.add(const GoogleSignIn()),
      expect: () => [isA<SignUpStarted>()],
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
      'failure emits SignUpFailed',
      build: () { when(() => mockRepo.googleSignIn()).thenAnswer((_) async => 'Google Sign-In cancelled'); return AuthenticationBloc(repo: mockRepo); },
      act: (bloc) => bloc.add(const GoogleSignIn()),
      expect: () => [isA<SignUpFailed>()],
    );
    blocTest<AuthenticationBloc, AuthenticationState>(
      'null result emits SignUpFailed',
      build: () { when(() => mockRepo.googleSignIn()).thenAnswer((_) async => null); return AuthenticationBloc(repo: mockRepo); },
      act: (bloc) => bloc.add(const GoogleSignIn()),
      expect: () => [isA<SignUpFailed>()],
    );
  });
