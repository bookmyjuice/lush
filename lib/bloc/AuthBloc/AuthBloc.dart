...
class AuthenticationBloc extends Bloc<AuthenticationEvent, AuthenticationState> {
  ...
  AuthenticationBloc() : super(AuthenticationInProgress()) {
    ...
    on<EnterEmail>((event, emit) async {
      ...
      // TODO: Call backend to send email verification code
      // For now, emit that verification code was sent
    ...
    on<VerifyEmail>((event, emit) async {
      ...
      // TODO: Call backend to verify email code
      ...
      if (event.verificationCode.length == 6) {
        ...
        emit(const EmailVerificationFailed(
            error: 'Invalid verification code. Please enter the 6-digit code.'));
      }
    });
    ...
    on<GoogleSignIn>((event, emit) async {
      await userRepository.googleSignIn_().then((res) {
        if (res == null) {
          emit(const SignUpFailed(
