...
class _ResetPasswordEmailScreenState extends State<ResetPasswordEmailScreen> {
  ...
  Future<void> _verifyCode() async {
    if (_code.length != 6) {
      _showToast('Invalid Code', 'Please enter a 6-digit code', ToastificationType.error);
      return;
    }

    setState(() => _isLoading = true);

    final response = await userRepository.verifyEmailCode(_email, _code);

    setState(() => _isLoading = false);

    if (response.contains('Success') || response.contains('verified')) {
      _showToast('Email Verified', 'You can now set a new password', ToastificationType.success);
      setState(() => _isCodeVerified = true);
    } else {
      _showToast('Invalid Code', response, ToastificationType.error);
    }
  }
  ...
  Future<void> _resendCode() async {
    ...
    final response = await userRepository.sendEmailVerification(_email);

    setState(() => _isLoading = false);

    if (response.contains('Success') || response.contains('sent')) {
      _showToast('Code Resent', 'A new code has been sent to your email', ToastificationType.success);
      _startResendTimer();
    } else {
  ...
  Future<void> _resetPassword() async {
    ...
    final response = await userRepository.resetPasswordViaEmail(
      email: _email,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isCodeVerified ? 'Set New Password' : 'Verify Email'),
  ...
  Widget _buildCodeSection() {
    return Column(
      ...
      children: [
        SizedBox(height: 30),
        Icon(Icons.email_outlined, size: 80, color: Colors.amber),
        SizedBox(height: 20),
        Text(
          'Verify Your Email',
        ...
        SizedBox(height: 30),
        Text('Enter Code', style: FontUtils.bodyText(color: Colors.black87, fontSize: 16, fontWeight: FontWeight.w600)),
        SizedBox(height: 16),
        PinInputTextField(
          pinLength: 6,
          autoFocus: true,
